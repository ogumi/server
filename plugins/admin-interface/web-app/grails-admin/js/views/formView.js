										...
										if (grailsadminRemote && grailsadminRemote === 'enabled') {
												...
												$.ajax({
														...
														.fail(function (result) {
																var errorMsg = "There was a problem when requesting " + url + ". ";
																var errors = []
																...
																alertErr = $("#msg-error")
																$("#confirm").modal("hide");
																alertErr.find(".alert-text").text(errorMsg);
																alertErr.fadeIn(100);
																setTimeout(function () {
																	alertErr.fadeOut();
																	if(errors.length > 0){
																		for(var i = 0; i < errors.length; i++) {
																			window.ParsleyUI.removeError(searchFieldInstance(that, errors[i].field), errors[i].field);
																		}
																	}
																}, 5000);
